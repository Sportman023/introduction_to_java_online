...
/**
 * Задача: Создать класс Book, спецификация которого приведена ниже. Определить
...
public class SimpleClassesAndObjectsTask09 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < book.length; i++) {
			book[i] = new SimpleClassesAndObjectsTask09().new Book(names[i], authors[i], publishingHouse[i],
					publishingYear[i], pagesNumber[i], price[i], bindingType[i]);
		}
	...
	private class Book {
		...
		public Book(String name, String authors, String publishingHouse, int publishingYear, int pagesNumber, int price,
				String bindingType) {
			id = idCounter++;
			this.name = name;
		...
		public String toString() {
			return id + " " + name + " " + authors + " " + publishingHouse + " " + publishingYear + " " + pagesNumber
					+ " " + price / 100.0 + " " + bindingType;
		}
	}
